...
def main() -> None:
    ...
    backend = subprocess.Popen([sys.executable, "-m", "uvicorn", "app.main:app", "--host", "127.0.0.1", "--port", "8000"], cwd=ROOT / "backend", env=environment, creationflags=NO_WINDOW)
    npm_command = ["npm.cmd" if WINDOWS else "npm", "run", "dev", "--", "--host", "127.0.0.1"]
    frontend = None
    try:
        ...
        print("Pune demo running at http://localhost:5173. Press Ctrl+C to stop both servers.")
